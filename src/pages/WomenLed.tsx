  ...
  const columns: Column<any>[] = [
    {
      ...
      render: (item) => (
        <div>
          <p className="font-medium" style={{ color: 'var(--text-primary)' }}>{item.brandName}</p>
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{item.city}{item.area ? `, ${item.area}` : ''}</p>
        </div>
      ),
    },
    {
      ...
      render: (item) => (
        <div>
          <p className="text-sm" style={{ color: 'var(--text-primary)' }}>{item.user?.name || '—'}</p>
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Gender: {item.user?.gender || '—'}</p>
        </div>
      ),
    },
    {
    ...
    },
    {
      key: 'createdAt',
      header: 'Applied',
      render: (item) => <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{formatDate(item.createdAt)}</span>,
    },
    {
      key: 'actions',
      header: 'Actions',
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="card p-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-purple-100/10 flex items-center justify-center">
                <Users className="w-5 h-5 text-purple-600" />
              </div>
              <div>
                <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{analytics.totalApproved}</p>
          ...
          <div className="card p-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-amber-100/10 flex items-center justify-center">
                <TrendingUp className="w-5 h-5 text-amber-600" />
              </div>
              <div>
                <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{analytics.totalPending}</p>
          ...
          <div className="card p-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-emerald-100/10 flex items-center justify-center">
                <BarChart3 className="w-5 h-5 text-emerald-600" />
              </div>
              <div>
                <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>{analytics.percentageOfPlatform}%</p>
                <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Of Platform</p>
              </div>
            </div>
          </div>
          <div className="card p-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-blue-100/10 flex items-center justify-center">
                <TrendingUp className="w-5 h-5 text-blue-600" />
              </div>
              <div>
                <p className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>+{analytics.newApprovedThisMonth}</p>
      ...
      {analytics?.ratingComparison && (
        <div className="card p-4">
          <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--text-primary)' }}>Rating Comparison</h3>
          <div className="flex items-center gap-6">
            <div>
              <span className="text-lg font-bold text-purple-600">{analytics.ratingComparison.womenLedAvgRating}★</span>
              <span className="text-xs ml-1" style={{ color: 'var(--text-muted)' }}>Women-Led Avg</span>
            </div>
            <div style={{ color: 'var(--border-strong)' }}>vs</div>
            <div>
              <span className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{analytics.ratingComparison.platformAvgRating}★</span>
              <span className="text-xs ml-1" style={{ color: 'var(--text-muted)' }}>Platform Avg</span>
            </div>
          </div>
        </div>
      )}
      ...
      {analytics?.categoryDistribution && analytics.categoryDistribution.length > 0 && (
        <div className="card p-4">
          <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--text-primary)' }}>Top Categories</h3>
      ...
      <div className="card overflow-hidden">
        <div className="px-4 py-3 border-b" style={{ borderColor: 'var(--border-light)' }}>
          <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Pending Women-Led Approvals</h3>
        </div>
        <DataTable
          columns={columns}
